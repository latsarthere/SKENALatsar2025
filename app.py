        ...
        for keyword_raw in kata_kunci_list:
            ...
            try:
                ...
                with debug_container:
                    st.write(f"---")
                    st.write(f"**Mencoba Query:** `{search_query}`")
                    st.write(f"**Rentang Tanggal:** `{tanggal_awal}` s/d `{tanggal_akhir}`")
                
                search_results = gn.search(search_query, from_=tanggal_awal, to_=tanggal_akhir)
                
                with debug_container:
                    st.info(f"**Jumlah hasil ditemukan:** `{len(search_results['entries'])}` berita.")
                # --- [AKHIR KODE DETEKTIF] ---
...
# --- Blok Halaman dan Navigasi (Tidak ada perubahan) ---
