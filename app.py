...
def ringkas_dengan_gemini(text: str, wilayah: str, usaha: str) -> str:
    ...
    prompt = (f"Analisis teks berikut. Buat ringkasan padu dalam 1-2 kalimat (maksimal 40 kata) yang fokus pada topik '{usaha}' di wilayah '{wilayah}'. "
...
# --- [FUNGSI FINAL] Mendapatkan URL Asli yang Lengkap ---
def get_real_url(gn_link):
    """
    Ambil URL asli dan LENGKAP dari link Google News dengan beberapa metode.
    """
    try:
        ...
        # METODE 2: Lakukan request untuk mengikuti redirect secara cerdas
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        with requests.Session() as s:
            s.headers.update(headers)
            # Gunakan `s.get` yang akan menangani redirect secara otomatis
            resp = s.get(gn_link, timeout=10, allow_redirects=True)
            # `resp.url` akan berisi URL final setelah semua redirect selesai
            return resp.url
...
def get_rentang_tanggal(tahun: int, triwulan: str, start_date=None, end_date=None):
    if triwulan == "Tanggal Custom":
        if start_date and end_date: return start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')
        return None, None
    triwulan_map = {
        "Triwulan 1": (f"{tahun}-01-01", f"{tahun}-03-31"), "Triwulan 2": (f"{tahun}-04-01", f"{tahun}-06-30"),
        "Triwulan 3": (f"{tahun}-07-01", f"{tahun}-09-30"), "Triwulan 4": (f"{tahun}-10-01", f"{tahun}-12-31"),
    }
...
def start_scraping(tanggal_awal, tanggal_akhir, kata_kunci_lapus_df, kata_kunci_daerah_df, start_time, table_placeholder, keyword_placeholder):
    ...
    for kategori, kata_kunci_list in kata_kunci_lapus_dict.items():
            ...
            try:
                ...
                for entry in search_results['entries']:
                    ...
                    st.session_state.hasil_scraping.append({
                        ...
                        "Kata Kunci": keyword, "Kategori": kategori
                    })
        ...
        if st.session_state.hasil_scraping:
            ...
            kolom_urut = ["Nomor", "Kata Kunci", "Judul", "Link", "Tanggal", "Sumber", "Ringkasan AI", "Teks Mentah"]
...
# --- Blok Halaman dan Navigasi (Sama seperti sebelumnya) ---
def set_page(page_name): st.session_state.page = page_name
def show_home_page():
    st.image("logo skena full.png", use_container_width=True)
    st.markdown("---")
    st.markdown("<div style='text-align: justify;'>Hallo! Sistem Scraping Konawe Selatan (SKENA) merupakan alat bantu BPS Kabupaten Konawe Selatan dalam menyediakan data statistik yang lengkap. Sistem ini melakukan pencarian (<i>scraping</i>) fenomena pendukung dalam bentuk berita di Google.</div>", unsafe_allow_html=True)
    st.markdown("<div style='text-align: justify; margin-top: 10px;'>Sebelum mengakses fitur utama, sangat disarankan untuk membaca bagian <b>Pendahuluan</b> terlebih dahulu.</div>", unsafe_allow_html=True)
    ...
    with col1_btn:
        st.subheader("📈 Neraca"); 
        if st.button("Pilih Neraca", key="home_neraca", use_container_width=True, disabled=is_disabled): st.session_state.page = "Scraping"; st.session_state.sub_page = "Neraca"; st.rerun()
    with col2_btn:
        st.subheader("👥 Sosial")
        if st.button("Pilih Sosial", key="home_sosial", use_container_width=True, disabled=is_disabled): st.session_state.page = "Scraping"; st.session_state.sub_page = "Sosial"; st.rerun()
    with col3_btn:
        st.subheader("🌾 Produksi")
        if st.button("Pilih Produksi", key="home_produksi", use_container_width=True, disabled=is_disabled): st.session_state.page = "Scraping"; st.session_state.sub_page = "Produksi"; st.rerun()
    with col4_btn:
        st.subheader("📑 Lainnya")
        if st.button("Pilih Lainnya", key="home_lainnya", use_container_width=True, disabled=is_disabled): st.session_state.page = "Scraping"; st.session_state.sub_page = "Lainnya"; st.rerun()
...
def show_documentation_page():
    ...
    folder_id = "1z1_w_FyFmNB7ExfVzFVc3jH5InWmQSvZ"; folder_url = f"https://drive.google.com/drive/folders/{folder_id}"
...
def show_scraping_page():
    ...
    if st.session_state.sub_page in ["Sosial", "Produksi", "Lainnya"]:
        st.header("Segera Hadir!"); st.info(f"Fitur scraping untuk data **{st.session_state.sub_page}** sedang dalam pengembangan."); st.balloons(); return
    ...
    if df_lapus is None or df_daerah is None: st.error("Gagal memuat data kata kunci. Aplikasi tidak dapat berjalan."); return
    st.success("✅ Data kata kunci berhasil dimuat."); original_categories = df_lapus.columns.tolist(); st.header("Atur Parameter Scraping")
    tahun_sekarang = date.today().year
    tahun_list = ["--Pilih Tahun--"] + list(range(2020, tahun_sekarang + 1)); tahun_input = st.selectbox("Pilih Tahun:", options=tahun_list)
    triwulan_list = ["--Pilih Triwulan--", "Triwulan 1", "Triwulan 2", "Triwulan 3", "Triwulan 4", "Tanggal Custom"]; triwulan_input = st.selectbox("Pilih Triwulan:", options=triwulan_list)
    start_date_input, end_date_input = None, None
    if triwulan_input == "Tanggal Custom":
        col1, col2 = st.columns(2)
        with col1: start_date_input = st.date_input("Tanggal Awal", date.today() - timedelta(days=30))
        with col2: end_date_input = st.date_input("Tanggal Akhir", date.today())
    ...
    if mode_kategori == 'Pilih Kategori Tertentu':
        kategori_terpilih = st.multiselect('Pilih kategori untuk diproses:', options=original_categories, max_selections=3)
    ...
    if st.button("🚀 Mulai Scraping", use_container_width=True, type="primary", disabled=is_disabled):
        ...
        if tanggal_awal and tanggal_akhir:
            ...
            end_time = time.time(); total_duration_str = f"{int((end_time - start_time) // 60)} menit {int((end_time - start_time) % 60)} detik"
            ...
            if not hasil_df.empty:
                ...
                kolom_tersedia = [col for col in kolom_final if col in hasil_df.columns]; output_df = hasil_df[kolom_tersedia]
                output = io.BytesIO()
                with pd.ExcelWriter(output, engine='openpyxl') as writer: output_df.to_excel(writer, sheet_name="Hasil Scraping", index=False)
                ...
                st.download_button("📥 Unduh Hasil Scraping (Excel)", output.getvalue(), file_name=nama_file_baru, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            else:
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        with st.form("login_form"):
            st.header("Login"); username = st.text_input("Username"); password = st.text_input("Password", type="password")
            if st.form_submit_button("Login", use_container_width=True, type="primary"):
                if username == "user7405" and password == "bps7405": st.session_state.logged_in = True; st.rerun()
                else: st.warning("Username atau password salah.")
    else:
