...
# --- [DIUBAH] Fungsi untuk mendapatkan URL asli ---
@st.cache_data
def get_original_url(google_url):
    """Mengikuti redirect dari URL Google untuk mendapatkan URL berita asli."""
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        response = requests.head(google_url, allow_redirects=True, timeout=10, headers=headers)
        return response.url
        ...
        for keyword_raw in kata_kunci_list:
            ...
            try:
                ...
                for entry in search_results['entries']:
                    ...
                    # --- [DIUBAH] Dapatkan URL asli ---
                    link_asli = get_original_url(link_google)
                    
                    if any(d['Link'] == link_asli for d in st.session_state.hasil_scraping): continue

                    judul = entry.title
                    ringkasan = ambil_ringkasan(link_asli)
                    ...
                    if lokasi_ditemukan or keyword_ditemukan:
                        ...
                        st.session_state.hasil_scraping.append({
                            ...
                            "Link": link_asli, "Tanggal": tanggal_str, "Ringkasan": ringkasan
                        })
            except Exception:
                continue
...
def show_home_page():
    ...
    st.markdown("_Sebelum mengakses fitur utama, sangat disarankan untuk membaca bagian **Pendahuluan** terlebih dahulu._")
...
def show_pendahuluan_page():
    ...
    st.markdown("Aplikasi ini dirancang untuk membantu dalam pengumpulan data berita online...")
    if not st.session_state.get('logged_in', False):
...
def show_documentation_page():
    ...
    # --- [DIUBAH] Tampilan Drive langsung, tanpa expander ---
...
def show_scraping_page():
    ...
    if df_lapus is None or df_daerah is None:
        st.error("Gagal memuat data kata kunci."); return
    ...
    if st.button("🚀 Mulai Scraping", use_container_width=True, type="primary", disabled=is_disabled):
        ...
        if tanggal_awal and tanggal_akhir:
            ...
            end_time = time.time()
            total_duration_str = f"{int((end_time - start_time) // 60)}m {int((end_time - start_time) % 60)}d"

            st.header("✅ Proses Selesai"); st.success(f"Scraping selesai dalam {total_duration_str}.")
            ...
            if not hasil_df.empty:
                ...
                # --- [DIUBAH] Format nama file dinamis ---
                nama_kategori_file = st.session_state.sub_page.replace(" ", "")
                triwulan_file = triwulan_input.replace(" ", "")
                timestamp = time.strftime("%Y%m%d-%H%M%S")
                nama_file_final = f"Hasil_Scraping_{nama_kategori_file}_{triwulan_file}_{tahun_input}_{timestamp}.xlsx"
                
                st.download_button(
                    label="📥 Unduh Hasil Scraping (Excel)",
                    data=output.getvalue(), file_name=nama_file_final,
